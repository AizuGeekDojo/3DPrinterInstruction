...
Guider Ⅱだけでなく、全ての積層型3Dプリンタにおいて言える事であるが、ヘッドの金属部とプラットフォームは動作中非常に熱くなるので火傷に注意する事。

2. 暖かい部屋で使用する

3Dプリンタのモータは低温な環境で動作させるとガタガタ言ってうまく動かないことがあるので、Dojoが寒い場合は暖房をつけて動作させる事。(目安として15度以上)
...
出力が開始されると、画面右下にステータスが表示される。ここまで進めば、PCはシャットダウンしても問題はない。
